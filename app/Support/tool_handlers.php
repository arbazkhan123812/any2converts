...
function getSignPdfHTML() {
    ...
    <div class="space-y-6">
        <div style="">
            <h2 class="text-3xl font-black text-slate-900 dark:text-white">Sign PDF Online Free</h2>
            <p>Use this tool to sign PDF files online free. You can type your signature, draw it, or upload an image, and place it on any page.</p>
            <p>Fast, secure, and privacy-friendly. Process your PDF securely in your browser and download the signed document instantly.</p>
        </div>
        <div class="rounded-2xl border border-emerald-200/70 bg-emerald-50/80 dark:bg-emerald-950/30 dark:border-emerald-900 p-4">
            <div class="font-semibold text-emerald-900 dark:text-emerald-100">Simple PDF Signer</div>
            <p class="mt-1 text-sm text-emerald-800 dark:text-emerald-200">Upload a PDF, preview the page, choose a signature type, drag it onto the page, and save.</p>
        </div>
        <div class="rounded-2xl border border-gray-200 dark:border-gray-700 p-5 space-y-4 bg-white dark:bg-gray-900">
            <input type="file" id="a2sPdfFile" class="w-full p-4 bg-white text-gray-900 dark:bg-gray-800 dark:text-gray-100 rounded-xl border border-gray-200 dark:border-gray-600" accept=".pdf,application/pdf">
            <div id="a2sStatus" class="hidden text-sm text-center"></div>
            
            <div id="a2sControls" class="hidden space-y-4">
                <div class="grid md:grid-cols-2 gap-4">
                    <select id="a2sPageSelect" class="w-full p-4 bg-white text-gray-900 dark:bg-gray-800 dark:text-gray-100 rounded-xl border border-gray-200 dark:border-gray-600"></select>
                    <div class="flex items-center gap-2">
                        <button type="button" id="a2sPrev" class="w-full bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-100 py-3 rounded-xl font-semibold hover:bg-slate-200 dark:hover:bg-slate-700 transition">Prev</button>
                        <button type="button" id="a2sNext" class="w-full bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-100 py-3 rounded-xl font-semibold hover:bg-slate-200 dark:hover:bg-slate-700 transition">Next</button>
                    </div>
                </div>
                
                <div class="rounded-3xl border border-gray-200 dark:border-gray-700 bg-slate-50 dark:bg-slate-950/40 p-4">
                    <div id="a2sStageWrap" class="overflow-auto rounded-2xl border border-gray-200 dark:border-gray-700 bg-slate-100 dark:bg-slate-900 p-3">
                        <div class="min-w-max mx-auto">
                            <div id="a2sStage" class="relative bg-white shadow-lg overflow-visible touch-none select-none"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <!-- Typed Signature -->
                    <div class="rounded-2xl border border-gray-200 dark:border-gray-700 p-4 space-y-4 bg-white dark:bg-gray-900">
                        <div class="font-semibold text-gray-900 dark:text-gray-100">Typed Signature</div>
                        <input id="a2sTypedText" class="w-full p-3 bg-white text-gray-900 dark:bg-gray-800 dark:text-gray-100 rounded-xl border border-gray-200 dark:border-gray-600" value="Your Name" placeholder="Type your name">
                        <div class="grid grid-cols-2 gap-3">
                            <input id="a2sFontSize" type="number" min="10" max="96" value="34" class="w-full p-3 bg-white text-gray-900 dark:bg-gray-800 dark:text-gray-100 rounded-xl border border-gray-200 dark:border-gray-600" placeholder="Size">
                            <input id="a2sColor" type="color" value="#111827" class="w-full h-12 p-1 bg-white text-gray-900 dark:bg-gray-800 dark:text-gray-100 rounded-xl border border-gray-200 dark:border-gray-600">
                        </div>
                        <button type="button" id="a2sAddTyped" class="w-full bg-emerald-600 text-white py-3 rounded-xl font-semibold hover:bg-emerald-700 transition">Add Typed Sign</button>
                    </div>

                    <!-- Draw Signature -->
                    <div class="rounded-2xl border border-gray-200 dark:border-gray-700 p-4 space-y-4 bg-white dark:bg-gray-900">
                        <div class="font-semibold text-gray-900 dark:text-gray-100">Draw Signature</div>
                        <div class="border border-gray-300 dark:border-gray-600 rounded-xl bg-white overflow-hidden">
                            <canvas id="a2sPad" class="w-full h-28 touch-none block"></canvas>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <button type="button" id="a2sClearPad" class="w-full bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-100 py-3 rounded-xl font-semibold hover:bg-slate-200 dark:hover:bg-slate-700 transition">Clear</button>
                            <button type="button" id="a2sAddDrawn" class="w-full bg-emerald-600 text-white py-3 rounded-xl font-semibold hover:bg-emerald-700 transition">Add Drawn Sign</button>
                        </div>
                    </div>
                </div>

                <div class="rounded-2xl border border-gray-200 dark:border-gray-700 p-4 space-y-4 bg-white dark:bg-gray-900">
                    <div class="font-semibold text-gray-900 dark:text-gray-100">Upload Signature Image</div>
                    <input type="file" id="a2sImageFile" class="hidden" accept="image/png,image/jpeg,image/webp">
                    <div class="grid md:grid-cols-2 gap-3">
                        <button type="button" onclick="document.getElementById(\'a2sImageFile\').click()" class="w-full bg-slate-800 text-white py-3 rounded-xl font-semibold hover:bg-slate-900 transition">Choose Image & Add</button>
                        <div class="flex gap-2">
                            <button type="button" id="a2sDelete" class="flex-1 bg-rose-600 text-white py-3 rounded-xl font-semibold hover:bg-rose-700 transition">Delete Selected</button>
                            <button type="button" id="a2sUndo" class="flex-1 bg-slate-500 text-white py-3 rounded-xl font-semibold hover:bg-slate-600 transition">Undo</button>
                        </div>
                    </div>
                </div>

                <div class="rounded-2xl border border-emerald-100 bg-emerald-50/70 dark:bg-emerald-950/20 dark:border-emerald-900 p-4 text-sm text-emerald-900 dark:text-emerald-100">
                    How to sign PDF:
                    Upload the file, choose a page, select a signature method above, drag the inserted signature to the correct spot, and click Save.
                </div>
                
                <button type="button" id="a2sSave" class="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-emerald-700 transition shadow-lg">Save Signed PDF</button>
            </div>
            <div id="a2sEmpty" class="rounded-2xl border border-dashed border-gray-300 dark:border-gray-700 p-10 text-center text-sm text-gray-500">
                A preview of your PDF will appear here after upload.
            </div>
        </div>
    </div>
    
    <style>
        .a2c-sign-ov { position:absolute; border:1px solid transparent; cursor:move; user-select:none; box-sizing:border-box; }
        .a2c-sign-ov.selected { border-color:#16a34a; box-shadow:0 0 0 2px rgba(22,163,74,.25); }
        .a2c-sign-ov img { display:block; width:100%; height:100%; object-fit:contain; pointer-events:none; }
        .a2c-sign-text { font-family:"Brush Script MT","Segoe Script",cursive; font-style:italic; line-height:1; white-space:nowrap; }
        .a2c-sign-resize { position:absolute; width:14px; height:14px; right:-7px; bottom:-7px; border-radius:999px; background:#16a34a; border:2px solid #fff; cursor:nwse-resize; z-index:10; }
    </style>
    ...
    (function(){
        ...
        const fileInput = $("a2sPdfFile"), statusEl = $("a2sStatus"), controls = $("a2sControls"), pageSelect = $("a2sPageSelect"), stage = $("a2sStage"), empty = $("a2sEmpty");
        const typedText = $("a2sTypedText"), fontSize = $("a2sFontSize"), color = $("a2sColor");
        const pad = $("a2sPad"), imageFile = $("a2sImageFile");
        
        let pdfBytes=null, pdfDoc=null, pdfName="signed.pdf", currentPage=0, selectedId=null, history=[], pages=[], currentViewport=null;
        
        // Pad resizing fix
        function resizePad() {
            const rect = pad.parentElement.getBoundingClientRect();
            pad.width = rect.width;
            pad.height = 112; // 28 tailwind units
        }
        window.addEventListener("resize", resizePad);
        setTimeout(resizePad, 100);

        const padCtx = pad.getContext("2d");
        padCtx.lineWidth=4; padCtx.lineCap="round"; padCtx.strokeStyle="#111827";
        let drawing=false, padHasInk=false;
        function pos(e){
        ...
        function startDraw(e){ drawing=true; padHasInk=true; const p=pos(e); padCtx.beginPath(); padCtx.moveTo(p.x,p.y); e.preventDefault(); }
        function doDraw(e){ if(!drawing)return; const p=pos(e); padCtx.lineTo(p.x,p.y); padCtx.stroke(); e.preventDefault(); }
        function endDraw(){ drawing=false; }
        
        pad.addEventListener("pointerdown", startDraw);
        pad.addEventListener("pointermove", doDraw);
        window.addEventListener("pointerup", endDraw);
        pad.addEventListener("touchstart", startDraw, {passive: false});
        pad.addEventListener("touchmove", doDraw, {passive: false});
        window.addEventListener("touchend", endDraw);

        $("a2sClearPad").onclick = () => { padCtx.clearRect(0,0,pad.width,pad.height); padHasInk=false; };

        function setStatus(m, e){
            if(!m) { statusEl.classList.add("hidden"); return; }
            statusEl.textContent=m;
            statusEl.classList.remove("hidden");
            statusEl.classList.toggle("text-red-500", !!e);
            statusEl.classList.toggle("text-gray-500", !e);
        }
        function clamp(n,min,max){ return Math.max(min,Math.min(max,n)); }
        function uid(){ return "sig-"+Date.now()+"-"+Math.random().toString(16).slice(2); }
        function pageState(){ return pages[currentPage]||{signatures:[]}; }
        function selected(){ return pageState().signatures.find(o=>o.id===selectedId)||null; }
        function pushHistory(){ history.push(pages.map(p=>({signatures:p.signatures.map(o=>({...o}))}))); if(history.length>40) history.shift(); }
        function baseName(n){ return (n||"signed.pdf").replace(/\.pdf$/i,"")||"signed"; }
        function hexToRgb(hex){ const v=String(hex||"#111827").replace("#",""); const n=parseInt(v.length===3?v.split("").map(c=>c+c).join(""):v,16); return {r:((n>>16)&255)/255,g:((n>>8)&255)/255,b:(n&255)/255}; }
        
        function makeSig(type, extra){
            const o = Object.assign({id:uid(), type, x:0.1, y:0.1, w:0.25, h:0.08, text:typedText.value||"Your Name", fontSize:Number(fontSize.value||34), color:color.value||"#111827"}, extra||{});
            pushHistory(); pageState().signatures.push(o); selectedId = o.id; renderPage();
        }

        async function loadPdf(file){
            pdfName = file.name||"signed.pdf";
            pdfBytes = await file.arrayBuffer();
            pdfDoc = await pdfjsLib.getDocument({data:pdfBytes.slice(0)}).promise;
            pages = Array.from({length:pdfDoc.numPages}, ()=>({signatures:[]}));
            currentPage = 0; selectedId = null; history = [];
            
            pageSelect.innerHTML = "";
            pages.forEach((p,i) => { const opt = document.createElement("option"); opt.value = i; opt.textContent = "Page "+(i+1); pageSelect.appendChild(opt); });
            
            empty.classList.add("hidden"); controls.classList.remove("hidden");
            setStatus(""); await renderPage();
        }

        function goPage(i){
            if(!pages.length) return;
            currentPage = clamp(i,0,pages.length-1);
            selectedId = null; pageSelect.value = String(currentPage); renderPage();
        }
        ...
        async function renderPage(){
            ...
            const wrapWidth = $("a2sStageWrap").clientWidth - 16;
            const baseViewport = page.getViewport({scale: 1});
            const scale = Math.min(2.0, wrapWidth / baseViewport.width);
            ...
            pageState().signatures.forEach(o => drawSig(layer, o));
        }

        function drawSig(parent, o){
            const node = document.createElement("div");
            node.className = "a2c-sign-ov" + (o.id===selectedId?" selected":"");
            node.style.left = (o.x*currentViewport.width)+"px";
            node.style.top = (o.y*currentViewport.height)+"px";
            node.style.width = (o.w*currentViewport.width)+"px";
            node.style.height = (o.h*currentViewport.height)+"px";
            
            if(o.type==="typed"){
                const d = document.createElement("div");
                d.className = "a2c-sign-text";
                d.textContent = o.text;
                d.style.fontSize = Math.max(12, o.fontSize*(currentViewport.scale||1))+"px";
            ...
            const startMove = (ev) => {
                ...
                selectedId = o.id; pushHistory(); renderPage();
                const r = stage.getBoundingClientRect();
                const sx = ev.clientX || (ev.touches && ev.touches[0].clientX), sy = ev.clientY || (ev.touches && ev.touches[0].clientY);
                const ox = o.x, oy = o.y;
                const move = (e) => {
                    const cx = e.clientX || (e.touches && e.touches[0].clientX), cy = e.clientY || (e.touches && e.touches[0].clientY);
                    o.x = clamp(ox + (cx-sx)/r.width, 0, 1-o.w);
                    o.y = clamp(oy + (cy-sy)/r.height, 0, 1-o.h);
                    node.style.left = (o.x*currentViewport.width)+"px"; node.style.top = (o.y*currentViewport.height)+"px";
                };
                const up = () => { window.removeEventListener("pointermove",move); window.removeEventListener("pointerup",up); window.removeEventListener("touchmove",move); window.removeEventListener("touchend",up); renderPage(); };
                window.addEventListener("pointermove",move); window.addEventListener("pointerup",up);
                window.addEventListener("touchmove",move, {passive:false}); window.addEventListener("touchend",up);
            };
            node.addEventListener("pointerdown", startMove);
            node.addEventListener("touchstart", startMove, {passive:false});
            
            const h = document.createElement("span");
            h.className = "a2c-sign-resize";
            const startResize = (ev) => {
                ev.preventDefault(); ev.stopPropagation(); pushHistory();
                const r = stage.getBoundingClientRect();
                const sx = ev.clientX || (ev.touches && ev.touches[0].clientX), sy = ev.clientY || (ev.touches && ev.touches[0].clientY);
                const sw = o.w, sh = o.h;
                const move = (e) => {
                    const cx = e.clientX || (e.touches && e.touches[0].clientX), cy = e.clientY || (e.touches && e.touches[0].clientY);
                    o.w = clamp(sw + (cx-sx)/r.width, 0.04, 1-o.x);
                    o.h = clamp(sh + (cy-sy)/r.height, 0.025, 1-o.y);
                    node.style.width = (o.w*currentViewport.width)+"px"; node.style.height = (o.h*currentViewport.height)+"px";
                };
                const up = () => { window.removeEventListener("pointermove",move); window.removeEventListener("pointerup",up); window.removeEventListener("touchmove",move); window.removeEventListener("touchend",up); renderPage(); };
                window.addEventListener("pointermove",move); window.addEventListener("pointerup",up);
                window.addEventListener("touchmove",move, {passive:false}); window.addEventListener("touchend",up);
        ...
        async function addImage(file){
            ...
            const src = URL.createObjectURL(new Blob([bytes], {type:file.type||"image/png"}));
            makeSig("image", {src, mime:file.type||"image/png", imageBytes:bytes, w:0.3, h:0.1});
        }

        $("a2sAddTyped").onclick = () => { if(pdfDoc) makeSig("typed"); };
        $("a2sAddDrawn").onclick = () => {
            if(!pdfDoc) return;
            if(!padHasInk) return alert("Please draw a signature on the pad first.");
            makeSig("drawn", {src:pad.toDataURL("image/png"), mime:"image/png", w:0.3, h:0.12});
        };

        async function savePdf(){
            if(!pdfBytes) return alert("Please upload a PDF first.");
            const count = pages.reduce((n,p) => n+p.signatures.length, 0);
            if(!count) return alert("Please add at least one signature before saving.");
            
            setStatus("Saving signed PDF...");
            try {
                const outPdf = await PDFLib.PDFDocument.load(pdfBytes, {ignoreEncryption:true});
                const font = await outPdf.embedFont(PDFLib.StandardFonts.HelveticaOblique);
                
                for(let i=0; i<pages.length; i++){
                    const page = outPdf.getPage(i), pw = page.getWidth(), ph = page.getHeight();
                    for(const o of pages[i].signatures){
                        const x = o.x*pw, y = ph - o.y*ph - o.h*ph, w = o.w*pw, h = o.h*ph;
                        if(o.type==="typed"){
                            const c = hexToRgb(o.color), size = Math.max(8, o.fontSize);
                            page.drawText(String(o.text||""), {x:x+2, y:y+h-size, size, font, color:PDFLib.rgb(c.r,c.g,c.b), maxWidth:w});
                        } else {
                            let bytes = o.imageBytes;
                            if(!bytes && o.src && o.src.startsWith("data:")) bytes = Uint8Array.from(atob(o.src.split(",")[1]), c=>c.charCodeAt(0));
                            if(!bytes) continue;
                            const img = String(o.mime||"").includes("png")||o.type==="drawn" ? await outPdf.embedPng(bytes) : await outPdf.embedJpg(bytes);
                ...
                a.download = baseName(pdfName) + "_signed.pdf";
                a.click();
                setTimeout(() => URL.revokeObjectURL(a.href), 1000);
                setStatus("Signed PDF downloaded successfully.");
            } catch(err) {
                setStatus("Could not sign this PDF: " + err.message, true);
            }
        }

        fileInput.onchange = () => { const f = fileInput.files && fileInput.files[0]; if(f) loadPdf(f).catch(e => setStatus("Could not open PDF: "+e.message, true)); };
        pageSelect.onchange = () => goPage(Number(pageSelect.value||0));
        $("a2sPrev").onclick = () => goPage(currentPage-1);
        $("a2sNext").onclick = () => goPage(currentPage+1);
        imageFile.onchange = () => addImage(imageFile.files && imageFile.files[0]);
        $("a2sSave").onclick = savePdf;
        
        $("a2sDelete").onclick = () => {
            if(!selectedId) return;
            pushHistory();
            pages[currentPage].signatures = pageState().signatures.filter(o => o.id!==selectedId);
            selectedId = null; renderPage();
        };
        
        $("a2sUndo").onclick = () => {
            const prev = history.pop();
            if(!prev) return;
            prev.forEach((p,i) => { pages[i].signatures = p.signatures; });
            selectedId = null; renderPage();
        };
        
        const updateTyped = () => { const o = selected(); if(o && o.type==="typed"){ o.text = typedText.value; o.fontSize = Number(fontSize.value||34); o.color = color.value; renderPage(); } };
        typedText.oninput = updateTyped; fontSize.onchange = updateTyped; color.oninput = updateTyped;
    })();
    </script>
    ';
}
